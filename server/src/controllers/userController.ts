...
class UserController
{
    ...
    public async Update(req: Request, res: Response)
    {
        ...
        if(user.length === 0) res.status(404).send({text: "No existe ningun usuario con el id enviado"})
        
        const chapter = await db.query("SELECT * FROM chapter WHERE chapter.id_chapter="+req.body.id_chapter);
        if(chapter.length === 0) res.status(404).send({text: "No existe ningun captiulo con el id enviado"})
